 ...
 */
function getIdPrefix(filename, dirPath, language) {
  var suffix;

  if (language) { // strip off lang part from name, e.g. 'uusimaa-fi.csv'
    suffix = '-' + language + '.csv';
  } else {
    suffix = '.csv';
  }
  ...
  if (filename && dirPath) {
    ...
    if (filename.indexOf(dirPath) !== -1) {
      ...
      var prefix = _.replace(subpath, suffix, '');
 ...
 */
function createRecordStream( filePath, dirPath, language ){
  /**
  ...
  var idPrefix = getIdPrefix(filePath, dirPath, language);
  var documentStream = DocumentStream.create(idPrefix, stats, language);
 ...
 */
function createFullRecordStream(files, dirPath, language) {
  ...
  files.forEach( function forEach( filePath ){
    ...
    recordStream.append( function ( next ){
      ...
      next(createRecordStream( filePath, dirPath, language ) );
    });
  });
