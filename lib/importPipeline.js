...
function createFullImportPipeline( files, opts, streams){
  logger.info( 'Importing %s files.', files.length );

  var finalStream = streams.finalStream || peliasDbclient({
    merge: opts.merge,
  ...
  recordStream.create(files, opts.dirPath, opts.language)
    .pipe(streams.deduplicatorStream)
    .pipe(streams.adminLookupStream)
